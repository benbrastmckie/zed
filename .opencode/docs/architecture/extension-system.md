...
The extension system enables modular domain support that can be loaded and unloaded on demand. Extensions are self-contained packages containing agents, skills, rules, commands, and context files that integrate with the core .opencode/ system when loaded. Extensions can optionally declare dependencies on other extensions for shared resources.
...
3. **OpenCode Agnostic**: OpenCode sees only standard `.opencode/` structure
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│ Layer 2: .opencode/ Agent System                                  │
│ (standard OpenCode directory structure)                      │
│                                                                 │
│  agents/    skills/    rules/    commands/    context/          │
│  AGENTS.md (fully regenerated by generate_claudemd())          │
│  context/index.json (entries appended/removed per extension)   │
│                                                                 │
│  Consumer: OpenCode (sees only this layer)                  │
└─────────────────────────────────────────────────────────────────┘
```

**Vocabulary**:
- **Source**: Files living in `.opencode/extensions/{name}/` -- the authoritative extension definition
- **Loaded**: Files copied into `.opencode/` -- the runtime-active state visible to OpenCode

OpenCode has no knowledge of Layer 1. It only sees the standard `.opencode/` directory structure populated by the loader.
...
Each extension lives in `.opencode/extensions/{name}/`:

```
.opencode/extensions/{name}/
...
When extensions are loaded, state is tracked in `.opencode/extensions.json`:
...
```json
{
  "extensions": {
    "latex": {
      ...
      "source_dir": "$PROJECT_ROOT/.opencode/extensions/latex",
      "installed_files": [
        ".opencode/agents/latex-implementation-agent.md",
        ".opencode/agents/latex-research-agent.md"
      ],
      "installed_dirs": [
        ".opencode/context/project/latex"
      ],
      "merged_sections": {
...
**Note**: The `merged_sections` object contains only `index` (and optionally `settings`). It does NOT contain a `claudemd` key -- AGENTS.md is fully regenerated by `generate_claudemd()`, not tracked per-extension.
...
| `merge_targets` | object | yes | Files that get merged (AGENTS.md, index.json, settings.json) |
...
**Note on `section_id`**: The `section_id` field in `merge_targets.claudemd` is vestigial for the AGENTS.md use case. `generate_claudemd()` regenerates AGENTS.md completely from all loaded extension `EXTENSION.md` files; it does not use section markers.
...
| `copy_root_files(manifest, source_dir, target_dir)` | Files at target_dir root | Placed directly in .opencode/ root |
...
| `root_files` | `copy_root_files()` | Files at .opencode/ root |
...
| Function | Purpose |
|----------|---------|
| `generate_claudemd()` | Fully regenerate AGENTS.md from all loaded extensions |
| `inject_section()` | Insert section with markers (exists but NOT used for AGENTS.md) |
| `remove_section()` | Remove section from file (exists but NOT used for AGENTS.md) |
...
**AGENTS.md as a Computed Artifact**:

AGENTS.md is NOT maintained via section injection. It is fully recomputed by `generate_claudemd()` on every load and unload operation:
...
4. Concatenate all fragments into a complete AGENTS.md

This means `inject_section()` and `remove_section()` still exist in merge.lua but are **not called** for AGENTS.md management. The `section_id` in the manifest's `merge_targets.claudemd` is vestigial.

**Section Markers** (historical reference only -- not used for AGENTS.md):
```markdown
...
```lua
{
  ...
  global_extensions_dir = "$PROJECT_ROOT/.opencode/extensions",
...
6. Merge shared files (process_merge_targets):
   a. generate_claudemd() -- regenerate full AGENTS.md from all loaded extensions
...
**Note**: There is no `remove_section()` call during unload. `generate_claudemd()` at step 7 rebuilds AGENTS.md from scratch, naturally excluding the unloaded extension.
...
OpenCode has no knowledge of the extension system. It only sees the standard `.opencode/` directory structure.

When an extension is loaded:
- Agents appear in `.opencode/agents/`
- Skills appear in `.opencode/skills/`
- Rules appear in `.opencode/rules/`
- Commands appear in `.opencode/commands/`
- Context appears in `.opencode/context/`
- Routing info appears in AGENTS.md (regenerated by `generate_claudemd()`)
...
Extension files are tracked by git. Use `git checkout HEAD -- .opencode/extensions/{ext}/` to recover any extension file, or `git log --oneline -- .opencode/extensions/` to find when changes occurred.
