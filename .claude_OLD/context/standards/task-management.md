...
- **Task Type**: Task language (neovim|markdown|general|meta|latex|typst, auto-detected by description-clarifier or user-provided)
...
Override Type when:
- Task involves Neovim code → neovim
...
- Task is file-specific (e.g., "Fix bug in Foo.neovim")
...
- **Neovim routing and MCP validation**:
  - Neovim research requests **must** use the Neovim research subagent; Neovim implementation requests **must** use the Neovim implementation subagent, selected from the TODO `Type` field (or explicit flag override) rather than heuristics.
  - Validate Neovim MCP server availability against `.mcp.json` before dispatch; fail with a clear error if required servers are missing/unreachable or if the command is absent/returns non-zero during a health ping.
  - Default required Neovim server: `neovim-lsp` (stdio via `uvx neovim-lsp-mcp`); planned servers (`neovim-explore`, `loogle`, `neovim-search`) should raise a "planned/not configured" warning instead of proceeding silently.
...
-   [ ] Language field reflects the primary language for the work (e.g., `neovim`, `markdown`, `python`, `shell`, `json`, `meta`, `general`).
...
**Impact**: 
- Prevents proper routing to specialized agents (neovim-research-agent, neovim-implementation-agent, meta subagents)
...
1. Identify the primary language for the task:
   - Neovim code tasks → `- **Task Type**: neovim`
   - LaTeX document tasks → `- **Task Type**: latex`
   - Typst document tasks → `- **Task Type**: typst`
   - Documentation tasks → `- **Task Type**: markdown`
   - Meta work (agents, commands, context) → `- **Task Type**: meta`
   - General tasks → `- **Task Type**: general`
...
1. "Type field is required but could not be detected"
   - Solution: Add `--type neovim` (or markdown/general/meta) flag to /task command
   - Example: `/task "Fix bug in Foo.neovim" --type neovim`
...
1. "Task {number} missing required Type field"
   ...
   - Follow format: `- **Type**: {neovim|markdown|general|meta}`
