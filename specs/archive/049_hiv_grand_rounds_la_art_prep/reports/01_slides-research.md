# Talk Research Report: MXM LA-ART & LA-PrEP Program -- HIV Grand Rounds
...
**Template**: Two Column -- Navy (Layout 27)
**Status**: mapped
...
**Template**: Bullet -- Navy (Layout 24)
**Status**: mapped
...
- **100% virally suppressed at latest follow-up** [STAT CALLOUT -- large visual]
...
**Template**: Two Column -- White (Layout 30)
**Status**: mapped
...
**Template**: Bullet -- White (Layout 28)
**Status**: mapped
...
**Template**: Poll slide (custom layout)
**Status**: mapped
...
### Slide 14: Patient 2 -- Timeline Slide(s)

**Template**: Custom timeline layout (animated build, recreate from prior deck style)
**Status**: mapped -- needs design guidance for timeline animation

**Content**:
*Timeline design: Horizontal arrow (dark navy), dose circles above (Purple=initiation, Green=on-time, Orange=delayed), lab values below, gold callout boxes for key decisions.*
...
**Template**: custom content slide (may need 2 slides if content is too dense)
...
**Template**: Closing layout (custom)
**Status**: mapped
...
1. **Slide 14 (Patient 2 timeline)**: The animated timeline build requires design-level decisions about progressive reveal across sub-slides. The source describes the prior deck style in detail (horizontal arrow, colored dose circles, lab annotations below, gold callout boxes), but the assembly agent will need to decide how many sub-slides to create for the build animation. This is a design/implementation concern, not a content gap.

2. **Slide 21 (Patient 4)**: Content density is very high. May need to split across 2 slides -- one for demographics/OI history and one for pericardial history + IRIS question. The assembly agent should make this call based on available slide real estate.

3. **Poll Everywhere integration**: The source specifies Poll Everywhere for interactive polls but does not provide QR codes or embed URLs. These will need to be added by the presenter before the actual talk.

No content gaps requiring additional source material. No clarifying questions needed.

## Recommended Theme

**UCSF/ZSFG Custom Template** (primary) with **clinical-teal** as the color system.

The source material specifies a custom UCSF/ZSFG 16x9 PowerPoint template (`UCSF_ZSFG_Template_16x9.pptx`) with specific layout indices:
- Layout 11: Divider -- Navy
- Layout 12: Divider -- Teal (case/section dividers)
- Layout 24: Bullet -- Navy (content on dark background)
- Layout 27: Two Column -- Navy
- Layout 28: Bullet -- White (content on light background)
- Layout 30: Two Column -- White
- Layout 35: Section Header -- Teal

**Color palette** (per UCSF brand):
- Navy: ~#1B2A4A
- Teal: ~#0095A8
- White slides with navy/teal accents

**Typography** (per UCSF brand):
- Headers: Garamond 36pt
- Sub-headers: Arial 18pt
- Content: Arial 22pt (minimum 14pt)

**Design rules**:
- Sparse text: 4-7 bullets max, large font, breathing room
- Big stat callouts (100%, 93%) as visual anchors
- No patient initials -- use Patient 1, 2, 3, 4

The clinical-teal theme from the talk library aligns well but the UCSF template should take precedence for brand compliance.
